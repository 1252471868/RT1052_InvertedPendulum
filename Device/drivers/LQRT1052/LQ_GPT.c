...
void my_GPT_Init(GPT_Type *base,uint32_t us,bool enableirq)
{
  gpt_config_t  gptconfig;
  GPT_GetDefaultConfig(&gptconfig);
  GPT_Init(base,&gptconfig);
  //GPT_SetClockDivider(base,75);
  if(base==GPT1)
  {
    GPT_SetOutputCompareValue(base,kGPT_OutputCompare_Channel1, USEC_TO_COUNT(us, CLOCK_GetFreq(kCLOCK_IpgClk)/2));
    if(enableirq)
    {
    GPT_EnableInterrupts(base, kGPT_OutputCompare1InterruptEnable);
    NVIC_SetPriority(GPT1_IRQn,NVIC_EncodePriority(NVIC_GetPriorityGrouping(),3,2));
    EnableIRQ(GPT1_IRQn);
    }
