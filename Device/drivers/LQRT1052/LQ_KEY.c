...
{
    /* 设置上拉输入 */
    LQ_PinInit(D7,  PIN_MODE_INPUT_PULLUP, 1);
    LQ_PinInit(D8, PIN_MODE_INPUT_PULLUP, 1);
    LQ_PinInit(D9, PIN_MODE_INPUT_PULLUP, 1);
}
...
uint8_t KEY_Read(uint8_t mode)
{
    ...
    if(key_up && (LQ_PinRead(D7)==0 || LQ_PinRead(D8)==0 /* || LQ_PinRead(D9)==0 */))
    {
        delayms(10);   //消抖
        key_up=0;
        if(LQ_PinRead(D7)==0)
        ...
        else if(LQ_PinRead(D8)==0)
        {
            return 2;
        }

        // else if(LQ_PinRead(D9)==0)
        // {
        //     return 3;
        // }

    }
    if(LQ_PinRead(D7)==1 && LQ_PinRead(D8)==1 /* && LQ_PinRead(D9)==1 */)
    {
        key_up=1;
    }
    return 0;   //无按键按下
