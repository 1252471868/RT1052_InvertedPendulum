...
void ICM_Get_Arc(short *ax,short *ay,short *az,short *gx,short *gy,short *gz)
{
   short gx_old,gy_old,gz_old;

      double len;
      ICM_Get_Raw_data(ax,ay,az,&gx_old,&gy_old,&gz_old);
      len = sqrt(1.0*(*ax)*(*ax)+1.0*(*ay)*(*ay)+1.0*(*az)*(*az));
      *ax = (short)acos((double)(*ax)/len)*180.0/3.1416;
      *ay = (short)acos((double)(*ay)/len)*180.0/3.1416;
      *az = (short)acos((double)(*az)/len)*180.0/3.1416;
      //  ICM_Get_Gyroscope(&gx_old,&gy_old,&gz_old);
       *gx += gx_old;
       *gy += gy_old;
       *gz += gz_old;
       delayms(100);
