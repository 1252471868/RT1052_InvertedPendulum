...
uint8_t array_write[300];
void my_Test_Flash_Write()
{
    DisableInterrupts;
    SCB_DisableDCache();

    uint8_t array_read[10],temp;
        uint8_t txt[20];
    for(int i=0;i<sizeof(array_write);i++)
    array_write[i]=(uint8_t)i;
    FLASH_EraseSector(3000);
    FLASH_WriteBuff(3000,0,0,array_write,sizeof(array_write));
        FLASH_ReadBuff(3000,0,13,&temp,1);
    for(int i=4;i>=0;i--)
    {
        sprintf(txt,"%.1f",array_read[i]);
        LCD_P6x8Str(0,4-i,txt);
    }
}

void my_Test_Flash_Read()
{
        DisableInterrupts;
    SCB_DisableDCache();
    float array[10];
    uint8_t txt[20];
    FLASH_ReadBuff(3000,0,0,(uint8_t *)array,sizeof(array));
    for(int i=4;i>=0;i--)
    {
        sprintf(txt,"%.1f",array[i]);
        LCD_P6x8Str(0,4-i,txt);
    }
